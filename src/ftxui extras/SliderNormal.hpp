...
/**
* THIS CODE IS A SLIGHTLY MODIFIED COPY OF ftxui::Slider
*	a copy because ftxui::SliderBase is in a anonymous namespace
* 
* a ftxui slider but outputs a normalized value.
*/
...
namespace {
    ...
    template <class T>
    class SliderNormalBase : public ComponentBase {
    public:
        ...
        Element Render() override {
            ...
            const float percent = float(value_());
        ...
        bool OnEvent(Event event) final {
            ...
            value_() = util::clamp(value_(), float(-1), float(1));
            if (old_value != value_()) {
                return true;
            }
            ...
            if (captured_mouse_) {
                ...
                case Direction::Right: {
                    value_() = (min_() + (event.mouse().x - gauge_box_.x_min) *
                        (max_() - min_()) /
                        (gauge_box_.x_max - gauge_box_.x_min)) / max_();
                    break;
                }
                case Direction::Left: {
                ...
                case Direction::Down: {
                    value_() = (min_() + (event.mouse().y - gauge_box_.y_min) *
                        (max_() - min_()) /
                        (gauge_box_.y_max - gauge_box_.y_min)) / max_();
                    break;
                }
                case Direction::Up: {
                ...
                value_() = std::max(float(-1), std::min(float(1), value_()));
                return true;
            }
    ...
    class SliderNormalWithLabel : public ComponentBase {
    public:
        SliderNormalWithLabel(ConstStringRef label, Component inner)
...
Component Slider(ConstStringRef label,
    ...
    auto slider = Make<SliderNormalBase<int>>(option);
    return Make<SliderNormalWithLabel>(std::move(label), slider);
}
...
Component Slider(ConstStringRef label,
    ...
    option.max = max;
    option.increment = increment;
    auto slider = Make<SliderNormalBase<float>>(option);
    return Make<SliderNormalWithLabel>(std::move(label), slider);
}
Component Slider(ConstStringRef label,
    Ref<long> value,
    ...
    option.max = max;
    option.increment = increment;
    auto slider = Make<SliderNormalBase<long>>(option);
    return Make<SliderNormalWithLabel>(std::move(label), slider);
}
